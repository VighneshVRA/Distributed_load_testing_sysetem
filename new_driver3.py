...
def create_database():
    conn = sqlite3.connect('test_data3.db')
    cursor = conn.cursor()
...
def produce_heartbeat():
    while True:
        ...
        driver_producer.send('heartbeat', value=b'Heartbeat 3')
        time.sleep(10)
...
# Send the JSON string to the Kafka topic
            driver_producer_2.send('metrics3', value=metrics_json.encode('utf-8'))
