...
def create_database():
    conn = sqlite3.connect('test_data2.db')
    cursor = conn.cursor()
...
def produce_heartbeat():
    while True:
        ...
        driver_producer.send('heartbeat', value=b'Heartbeat  2')
        time.sleep(10)
...
# Send the JSON string to the Kafka topic
            driver_producer_2.send('metrics2', value=metrics_json.encode('utf-8'))
